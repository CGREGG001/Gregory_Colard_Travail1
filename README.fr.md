...
├── frontend/    → Futur client web (Angular)
...
- **Refresh Token Rotation** : Chaque utilisation du Refresh Token génère un nouveau couple Access/Refresh.
- **Guards NestJS** : Protection fine des routes (`JwtAuthGuard`, `RolesGuard`, `RefreshTokenGuard`).

Le frontend gère :
- **Le rafraîchissement automatique des tokens** via interceptor
- **La redirection** en cas de 401/403
- **Le logout + révocation du token en base de données**
<br/>
